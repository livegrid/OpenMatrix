...
description: >-
  ...
  device and whether to run a short serial monitor pass. Handles PlatformIO
  upload failures when the serial port is busy (Windows), ensures monitors are
  stopped cleanly, and caps retries after crashes. Use when finishing edits to
  OpenMatrix firmware (src/, include/, platformio.ini), memory or stack tuning,
  BLE, boot-critical paths, or any change where a flash-and-watch verification
  is expected.
---
...
If unsure, prefer a quick `pio run` (compile-only) to confirm the tree still builds; upload only when a device test is meaningful.
...
- **Build**: `pwsh -File .cursor/scripts/fw.ps1 build` (or `pio run`).
- **Upload**: `pwsh -File .cursor/scripts/fw.ps1 upload` (or `pio run -t upload`; use project default env unless the user specified another).
...
- **If the device crashes or misbehaves**:
   - **First failure**: Summarize logs; if the fix is obvious and low-risk (e.g., revert a single bad flag, fix a clear null dereference), apply it and repeat from build (counts as one retry round).
